...
<div class="wrapper">
    ...
    @endif
        ...
        <div class="box-header">
          <h3 class="box-title">Lists table peminjaman data</h3>
        </div>
        {!! Form::close() !!}
        <!-- /.box-header -->
      ...
      @include('peminjaman.modal_detail')
...
@endphp
      @foreach($peminjamanbarang as $data)
          <tr>
              ...
              <td style="vertical-align: middle;text-align: center">{{$data->no_pinjam}}</td>
              <td style="vertical-align: middle">{{ $data->id_pegawai }}</td>
              <td style="vertical-align: middle">{{ $data->tgl_pinjam}}</td>
              <td style="vertical-align: middle;text-align: center">{{$data->tgl_pengembalian}}</td>
              <td style="vertical-align: middle">{{ $data->keterangan }}</td>
              <td style="vertical-align: middle;text-align: center">
              @if($data->status_peminjaman == 1)
              <span class="label label-primary">sudah dipinjam</span>
              @else
              <span class="label label-warning">belum dipinjam</span>
              @endif
              </td>
              <td style="vertical-align: middle"><button type="button" class="btn btn-default" data-toggle="modal" data-target="#showdetail">
                show detail peminjaman
              </button></td>
              <td>
              <form id="frm_{{$data->no_pinjam}}"
                      action="#"
                      method="post" style="padding-bottom: 0px;margin-bottom: 0px">
                      <a class="btn btn-success btn-sm" title="Create"
                      href="{{url('peminjaman/create')}}">
                      <i class="fa fa-plus"></i></a>
                          <a class="btn btn-info btn-sm" title="Show"
                      href="{{url('peminjaman/show/'.$data->no_pinjam)}}"><i class="fa fa-search"></i></a>
                      {{csrf_field()}}
                  </form>
              @endforeach
  </td>
