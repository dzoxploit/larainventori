...
<div class="wrapper">
    ...
    <div class="content-wrapper">
        ...
        <div class="col-md-8 offset-md-2">
            ...
            <hr/>
            @if(isset($peminjaman))
                {!! Form::model($peminjaman,['method'=>'put']) !!}
            @else
                {!! Form::open() !!}
            @endif
        ...
        <div class="form-group row required">
            ...
            <div class="form-group row required">
                {!! Form::label("tanggal_pengembalian","tanggal pengembalian",["class"=>"col-form-label col-md-3 col-lg-2"]) !!}
                <div class="col-md-8">
          ...
          <td>{!! Form::textarea("nama_barang[]",null,["class"=>"form-control".($errors->has('nama_barang')?" is-invalid":""),"autofocus",'placeholder'=>'nama_barang']) !!}
            {!! $errors->first('nama_barang','<span class="invalid-feedback">:message</span>') !!}</td>

        <td>{!! Form::textarea("quantity[]",null,["class"=>"form-control".($errors->has('quantity')?" is-invalid":""),"autofocus",'placeholder'=>'stok terakhir barang']) !!}
            {!! $errors->first('quantity','<span class="invalid-feedback">:message</span>') !!}</td>

        <td>{!! Form::textarea("satuan[]",null,["class"=>"form-control".($errors->has('satuan')?" is-invalid":""),"autofocus",'placeholder'=>'satuan barang']) !!}
            {!! $errors->first('satuan','<span class="invalid-feedback">:message</span>') !!}</td>

        <td>{!! Form::textarea("jumlah_barang_pinjam[]",null,["class"=>"form-control".($errors->has('jumlah_barang_pinjam')?" is-invalid":""),"autofocus",'placeholder'=>'jumlah_barang_pinjam']) !!}
            {!! $errors->first('quantity','<span class="invalid-feedback">:message</span>') !!}</td>
...
<!-- Optionally, you can add Slimscroll and FastClick plugins.
    ...
    $(document).ready(function(){      
      ...
      $('#add').click(function(){  
           i++;  
           $('#dynamic_field').append('<tr id="row'+i+'" class="dynamic-added"><td><select class="form-control" name="id_barang[]"> @foreach($barang as $key => $value) <option value="{{$value}}">{{$key}}</option> @endforeach </select></td> <td>{!! Form::textarea("nama_barang[]",null,["class"=>"form-control".($errors->has('nama_barang')?" is-invalid":""),"autofocus",'placeholder'=>'nama_barang']) !!} {!! $errors->first('nama_barang','<span class="invalid-feedback">:message</span>') !!}</td><td>{!! Form::textarea("quantity[]",null,["class"=>"form-control".($errors->has('quantity')?" is-invalid":""),"autofocus",'placeholder'=>'stok terakhir barang']) !!}{!! $errors->first('quantity','<span class="invalid-feedback">:message</span>') !!}</td> <td>{!! Form::textarea("satuan[]",null,["class"=>"form-control".($errors->has('satuan')?" is-invalid":""),"autofocus",'placeholder'=>'satuan barang']) !!}{!! $errors->first('satuan','<span class="invalid-feedback">:message</span>') !!}</td><td>{!! Form::textarea("jumlah_barang_pinjam[]",null,["class"=>"form-control".($errors->has('jumlah_barang_pinjam')?" is-invalid":""),"autofocus",'placeholder'=>'jumlah_barang_pinjam']) !!}{!! $errors->first('quantity','<span class="invalid-feedback">:message</span>') !!}</td><td><button type="button" name="remove" id="'+i+'" class="btn btn-danger btn_remove">X</button></td></tr>');  
      });  
