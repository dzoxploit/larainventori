...
@endphp
      @foreach($barangs as $barangs)
          <tr>
              ...
              <td style="vertical-align: middle">{{ $barangs->id_barang }}</td>
              <td style="vertical-align: middle">{{ $barangs->nama_barang}}</td>
              <td style="vertical-align: middle">
              {{ $barangs->merk }}
              </td>
              <td style="vertical-align: middle">
              {{ $barangs->spesifikasi_barang }}
              </td>
              <td style="vertical-align: middle">
              {{ $barangs->id_kategori }}
              </td>
              <td style="vertical-align: middle">
              {{ $barangs->lokasi_barang }}
              </td>
              <td style="vertical-align: middle">
              {{ $barangs->quantity }}
              </td>
              <td style="vertical-align: middle">{{ $barangs->satuan }}</td>
              <td style="vertical-align: middle"> 
              @if($barangs->status == 1)
              ...
              <td>
              <form id="frm_{{$barangs->id_barang}}"
                      action="{{url('barang/delete/'.$barangs->id_barang)}}"
                      method="post" style="padding-bottom: 0px;margin-bottom: 0px">
                      <a class="btn btn-primary btn-sm" title="Edit"
                      href="{{url('barang/update/'.$barangs->id_barang)}}">
                      <i class="fa fa-edit"></i></a>
                          <a class="btn btn-info btn-sm" title="Show"
                      href="{{url('barang/show/'.$barangs->id_barang)}}"><i class="fa fa-search"></i></a>
                      <input type="hidden" name="_method" value="delete"/>
                      {{csrf_field()}}
                      <a class="btn btn-danger btn-sm" title="Delete"
                      href="javascript:if(confirm('Are you sure want to delete?')) $('#frm_{{$barangs->id_barang}}').submit()">
