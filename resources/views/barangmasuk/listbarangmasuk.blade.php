...
<head>
    <meta charset="UTF-8">
    <title>Barang masuk | List barang masuk</title>
...
@endphp
      @foreach($barangmasuk as $barangmasuk)
          <tr>
              <td style="vertical-align: middle;text-align: center">{{ $barangmasuk->id_masuk_barang }}</td>
              <td style="vertical-align: middle">{{ $barangmasuk->tanggal_masuk}}</td>
              <td style="vertical-align: middle">
              {{ $barangmasuk->jumlah_masuk }}
              </td>
              <td style="vertical-align: middle">
              {{ $barangmasuk->kode_supplier }}
              </td>
              <td>
              <form id="frm_{{$barangmasuk->id_masuk_barang}}"
                      action="#"
                      method="post" style="padding-bottom: 0px;margin-bottom: 0px">
                      <a class="btn btn-primary btn-sm" title="Edit"
                      href="{{url('barangmasuk/update/'.$barangmasuk->id_masuk_barang)}}">
                      <i class="fa fa-edit"></i></a>
                          <a class="btn btn-info btn-sm" title="Show"
                      href="{{url('barangmasuk/show/'.$barangmasuk->id_masuk_barang)}}"><i class="fa fa-search"></i></a>
